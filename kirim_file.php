...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nama !== '' && isset($_FILES['file_kitab']) && $_FILES['file_kitab']['error'] == UPLOAD_ERR_OK) {
        $tmp_name = $_FILES['file_kitab']['tmp_name'];
        $name_file = basename($_FILES['file_kitab']['name']);
        $name_file = preg_replace("/[^a-zA-Z0-9.-]/", "_", $name_file);
        $unique_name = time() . '_' . $name_file;
        ...
        if (move_uploaded_file($tmp_name, $destination)) {
            ...
            $err = "Gagal mengupload file ke server.";
        }
    } else {
        $err = "Nama dan File Kitab wajib diisi/dipilih.";
    }
}
?>
<!doctype html>
...
<body>
  <div class="wrap">
    <div class="card">
      <h1>Kirim File Kitab</h1>
      <p class="small">Punya file dokumen kitab (PDF/DOCX) atau teks yang ingin ditambahkan ke <span class="brand">Mafatihul Akhyar</span>? Kirimkan di sini.</p>

      <?php if ($ok): ?>
        <div class="alert ok">Terima kasih! File Anda telah berhasil diupload.</div>
      <?php elseif ($err): ?>
      ...
      <form class="form" method="post" action="" enctype="multipart/form-data">
        ...
        <div>
          <label for="file_kitab">File Dokumen <span class="brand">*</span></label>
          <input type="file" class="file-input" id="file_kitab" name="file_kitab" accept=".pdf,.doc,.docx,.txt" required />
        </div>
        <div>
          <label for="pesan">Pesan / Keterangan</label>
          <textarea class="input" id="pesan" name="pesan" placeholder="Tambahkan keterangan singkat tentang kitab ini (opsional)"></textarea>
        </div>
        <div class="actions">
      ...
      <p class="footer-note">Hanya menerima format dokumen yang didukung.</p>
    </div>
  </div>
</body>
</html>
